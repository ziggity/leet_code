...
// 0. Create a blank array - keep track of substrings: 1. Iterate over the length of string, and check the substring to previous one, find the max < length of string, return total count
...
var lengthOfLongestSubstring = function(s) {
    ...
    while (i < s.length) {
        if (cache[s[i]]) { // checks for repeating characters
            if (str.length > max) {
                max = str.length;
            }
            var start = str.indexOf(s[i]); // starat over with repeating character location
...
console.log(lengthOfLongestSubstring('dhdhddd')) // 2 (dh)
